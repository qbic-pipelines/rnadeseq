...
Table indicating which contrasts to consider. Each contrast is specified in one column, each row corresponds to the each of the expanded terms of the linear model. If not specified, default contrasts are calculated.
...
Path to `Software_versions.csv` file generated by the RNAseq pipeline.

### `--quote`

Path to the signed copy of the QBiC offer as pdf.
...
Check here an [example file](https://raw.githubusercontent.com/qbicsoftware/rnadeseq/dev/testdata/report_options.yml).
