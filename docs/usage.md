...
Table indicating which contrasts to consider. Each contrast is specified in one column, each row corresponds to the each of the expanded terms of the linear model. If not specified, default contrasts are calculated by DESeq2 (pairwise contrasts of all conditions).
