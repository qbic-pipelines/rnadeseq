...
DT::datatable(metadata, options = list(scrollX  = "100%", scrollY = "400px", pageLength = -1, dom = 'ft'))
```
...
The image below displays the analysis workflow which is a two-step process. 
In a first step, reads are aligned and assigned to genomic features using nf-core/pipelines.
In a second step, the gene counts are analyzed for differential expression with the downstream secondary analysis pipeline rnadeseq.
...
The primary bioinformatics analysis was performed using the [nf-core/rnaseq](https://nf-co.re/rnaseq) pipeline, revision ", 
...
The primary bioinformatics analysis was performed using the pipeline [nf-core/smrnaseq](https://nf-co.re/smrnaseq), revision ", 
...
The read sequencing quality was analyzed using [FASTQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/). Details about [software versions](./QC/multiqc_report.html#software_versions) of the primary bioinformatics analysis 
...
The table below was extracted from the `MultiQC` report and summarizes statistics from the primary bioinformatics analysis.
...
In gneral, most reads should be assigned. Assignment types:
...
cat(print(sampleDistMatrix[1:5,1:5]))
...
revision, `r as.character(params$revision)`.
...
<br><details>
<summary>Expand/collapse<br></summary>
...
helps to reduce the number of false postives (not real differentially expressed genes).

The linear model used in DESeq2 determines how to model gene expression:
```{r print_model}
cat(paste(readLines(params$path_design), collapse=" "))
```

In the resulting tables, the genes are marked as "DE" if they were found to be
...
The original plots, as well as interactive plots can be downloaded [here](./differential_gene_expression/plots/volcano_plots/).
...
for (file in allgenes_files){
    ...
    pg <- ggplot(
            ...
            geom_point(
            alpha = 0.5, aes(colour = ifelse(color == "red", paste0("DE:", "p_adj<", params$adj_pval_threshold, " & Log2FC >|", params$logFC_threshold, "|"),
                ifelse(color == "blue", paste0("p_adj<", params$adj_pval_threshold),
                ifelse(color == "green", paste0("log2FC>|", params$logFC_threshold, "|"),
                    ifelse(color == "black", "NS", "")
                )
                )
            )),
            na.rm = TRUE
            ...
            geom_vline(xintercept = 1, linewidth = 0.2, color = "grey") +
            scale_color_manual(name = "", values = c("red", "green", "black", "blue")) +
            scale_y_continuous(limits = c(-1, max(-log10(DE_genes[['padj']]), na.rm=TRUE) + 5)) +
            ylab("-log 10 padj") + xlab("Log2 Fold Change") +
            theme(panel.spacing = unit(ifelse(length(allgenes_files)>2, 0.1, 0.6), "lines")) +
            scale_x_continuous(breaks = seq(-200, 200, ifelse(log2FoldChange_quotient<3, 1, 5))) +
            ggtitle(
                paste('Comparison:', current_contrast_name),
                subtitle=paste0("DE results; total=", thousandFormat(nrow(DE_genes)), ", DE=", thousandFormat(numDE)))
    ...
    pg <- pg + ggtitle(
                paste('<b>Comparison:', current_contrast_name, '</b>',
                '<br><sup>',
                paste0("DE results; total=", thousandFormat(nrow(DE_genes)), ", DE=", thousandFormat(numDE))
                ,'</sup>'))
...
- `*_gost_pathway_venn_diagram.pdf/png`
    - Venn diagrams showing the numbers of enriched terms when using a background gene list vs when not using a bg list.
- `enrichment_plots/*_pathway_enrichment_plot.{pdf/png/svg}`
    - Barplots showing the proportion of differentially expressed genes in the pathway for a certain pathway database (might be missing if too many terms were enriched for fitting into a plot).
...
- `enrichd_heatmaps`
    - Contains heatmaps of the normalized DE gene counts for each enriched term.
</details><br>",
gost_text,
...
Only contrasts for which an enriched terms were found are shown.
Hover over the dots to display the pathway names. The table below provides more detail on all enriched terms."))
```
...
for (i in 1:length(q_names))  {
    ...
    if (length(q_list) > 0) {
        ...
        if (nrow(path_enrich) > 0){
            ...
            cat('#### Contrast: ', contrast_list[i], '\n')
            cat("\n\n")
            cat(paste("No enriched terms were found in the selected databases."))
            path_enrich=data.frame(list())
            cat("\n\n")
        }
    } else {
        cat("No DE genes were found, enrichment analysis not possible.")
        path_enrich=data.frame(list())
    }
...
You can find these heatmaps inside the [enrichment_analysis](./enrichment_analysis/heatmaps/) folder, inside the subfolder corresponding to each of the contrasts,
...
We analysed the differential expression of genes in the provided data and compared the following conditions:

Primary bioinformatics analysis was performed using the `r primary_pipeline` pipeline and gene counts were obtained with the quantification method `r quant_tool`.
...
The Nextflow-based [nf-core/rnaseq](https://nf-co.re/rnaseq) pipeline, revision ", as.character(ifelse(software_versions_csv, version["nf-core/rnaseq"], 
version["Workflow"][[1]]["nf-core/rnaseq"])),
", was used for the [primary bioinformatic analysis](#primary-bioinformatics-rnaseq). 
[FASTQC ", as.character(ifelse(software_versions_csv, version["FastQC"], version["FASTQC"][[1]]["fastqc"])),
"](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) [@andrews2010fastqc] was used to determine the quality of the FASTQ files.
...
"](https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) [@krueger2012trim]. 
The [STAR v", as.character(ifelse(software_versions_csv, substring((version["STAR"]), 7, ), version["MAKE_TRANSCRIPTS_FASTA"][[1]]["star"])),
"](https://github.com/alexdobin/STAR) [@Dobin2013] aligner was used to map the reads that passed the quality control to the reference genome.
...
"](https://rseqc.sourceforge.net/) [@wang2012rseqc] and read quantification of the features (e.g. genes) with `",
quant_tool,
" ",
quant_version,
"` ",
...
The Nextflow-based [nf-core/smrnaseq](https://nf-co.re/smrnaseq/) pipeline, revision ", as.character(version["Workflow"][[1]]["nf-core/smrnaseq"]), 
" was used for the [primary bioinformatic analysis](#primary-bioinformatics-smrnaseq). 
[FASTQC ", as.character(version["FASTQC"][[1]]["fastqc"]),
"](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) [@andrews2010fastqc] was used to determine the quality of the miRNA samples. ",
...
"](https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) [@krueger2012trim]."),
    ...
    "](https://github.com/OpenGene/fastp) [@chen2018fastp; @chen2023ultrafast]."),
        "")),
" [Bowtie v", as.character(version["BOWTIE_MAP_HAIRPIN"][[1]]["bowtie"]), "](https://bowtie-bio.sourceforge.net/bowtie2/index.shtml) [@langmead2009ultrafast] 
...
"](https://github.com/friedlanderlab/mirtrace) [@kang2018mirtrace] and read quantification of the features (e.g. genes) with `",
quant_tool,
" ",
quant_version,
"` ",
...
cat(paste0("\nAn aggregation of the quality control for the RNAseq analysis was performed with [MultiQC ", as.character(version["MultiQC"]), 
"](https://seqera.io/multiqc/)[@ewels2016multiqc]."))
```
...
The secondary analysis of differential gene expression was performed by the using the [rnadeseq pipeline](https://github.com/qbic-pipelines/rnadeseq), revision ", 
...
quant_tool, "` was processed with the R package [DESeq2 v",
packageVersion("DESeq2"),
"](https://bioconductor.org/packages/release/bioc/html/DESeq2.html) [@love2014differential]. 
...
For the enrichment analysis, the R packages [gprofiler2 v", version_gprofiler2,
" ](https://biit.cs.ut.ee/gprofiler/page/r), `AnnotationDbi v", version_annotation,
...
All images in this report were generated in RStudio with ``r R.version.string`` mainly using the R package `ggplot2 v`r packageVersion("ggplot2")``. 
...
and `DT v`r packageVersion("DT")`` R packages.

## Package version overview

The versions of the software used for the analysis are saved in the [differential_gene_expression](./differential_gene_expression/rnadeseq_software_versions.yml) folder.
...
sessioninfo::session_info() %>%  write_yaml(paste0(wd, "/differential_gene_expression/rnadeseq_software_versions.yml"))
```
