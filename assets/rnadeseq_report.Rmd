...
In the second step, the gene counts were analyzed for differential expression with the downstream secondary analysis pipeline rnadeseq.
The workflow highlightes the pipelines and methods that were used in colors.
...
digraph flowchart {
  ...
  C [label = "Quantification", shape = diamond, fillcolor = "#f0f0f0", fontcolor = "#333333" width = 1.5, height = 1.5 ]
  ...
  G [label = "miRTop", fillcolor =  "%s", fontcolor = "#333333"]
...
The first contrast listed should be the experimental condition to test for and the contrast listed at the end should be used as the baseline (control) expression. 
...
Uncollapse the table below to show the meaning of different design strategies with example conditions:
...
| `~ treatment + genotype`         | Model additive effects of treatment and genotype (assumes independence of both ) |
...
| `~ batch + genotype * treatment` | Full model, adjusting for batch and testing interaction effects                  |
    ...
    pg <- ggplot(DE_genes,
            ...
            ggtitle(paste('Comparison:', current_contrast_name),
                    subtitle=paste0("DE results; total=", 
                        thousandFormat(nrow(DE_genes)), ", DE=", 
...
for (i in 1:length(q_names))  {
    ...
    if (length(q_list) > 0) {
        ...
        if (nrow(path_enrich) > 0){
            cat('#### Contrast: ', contrast_list[i], '\n')
            cat("\n")
            cat("Hover over the datapoints to display additional information.") 
            cat("\n\n")
            ...
            cat('#### Contrast: ', contrast_list[i], '\n')
            cat("\n\n")
            cat(paste("> **⚠️ Warning:** No enriched terms were found in the selected databases."))
...
We analysed the differential expression of genes in the provided data with the rnadeseq pipeline.
