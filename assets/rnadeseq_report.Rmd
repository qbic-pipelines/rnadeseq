...
<br>
Click to expand/uncollapse the details:s<br>
<details>
<summary>**enrichment_analysis**</summary>
- **metadata**:
    - `custom_background.txt`: computed by the pipeline; contains the background gene list without expression values that were excluded from the enrichment analysis.
...
- **DE_contrast_condition_**:
    ...
    - `DATABASE_pathway_enrichment_results.tsv`: Table with the enriched pathways for the specific database, containing the pathway name, code, number of DE genes in the pathway, total number of genes in the pathway, fraction of DE genes in the pathway and adjusted p-value.
    - `pathway_enrichment_results.tsv`: Table with the enriched pathways for all databases, containing the pathway name, code, number of DE genes in the pathway, total number of genes in the pathway, fraction of DE genes in the pathway and adjusted p-value.
</details><br>",
gost_text,
...
```{r quote, echo=FALSE, results='asis', include=F}
...
quote_string <- ifelse(
    ...
    paste0("Further support for this project will be restricted to the results presented in this report.
...
" [@krueger2012trim].
The [STAR](https://github.com/alexdobin/STAR)", as.character(ifelse(software_versions_csv, substring((version["STAR"]), 7, ), version["MAKE_TRANSCRIPTS_FASTA"][[1]]["star"])),
...
" [@krueger2012trim]."),
    ...
    " [@chen2018fastp; @chen2023ultrafast]."),
        "")),
" [Bowtie](https://bowtie-bio.sourceforge.net/bowtie2/index.shtml)", as.character(version["BOWTIE_MAP_HAIRPIN"][[1]]["bowtie"]), " [@langmead2009ultrafast]
...
quant_tool, "` was processed with the R package [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html) v",
