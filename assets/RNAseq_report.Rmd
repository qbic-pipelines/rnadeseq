...
```{r conditional_KEGG_contrasts, echo=FALSE, results='asis', eval=config$KEGG & contr}
cat("## KEGG pathway analysis
Enriched KEGG pathways were found using a pathway analysis with `gProfileR`. The results are stored in [this folder](./gProfileR/), divided in subfolders, one for each contrast. Each subfolders contains a list of the enriched pathways, stored in the file ending with `results.tsv`, and also the plots of the KEGG pathway maps of the enriched pathways, that can be found in the folder named `KEGG_pathways`; in these maps, red corresponds to upregulated and green to downregulated parts of the pathways. There hardly is a difference in colour intensity between ~ 0.5 and 2.5 fold change.
Shown below you find the KEGG pathways enrichment plot for every contrast:")
...
for (i in 1:length(contrnames)){
  ...
  path = paste0("/", dirname, "/", enriched_plot)
...
cat("## KEGG pathway analysis
Enriched KEGG pathways were found using a pathway analysis with `gProfileR`. The results are stored in [this folder](./gProfileR/) where you can also find a list of the enriched pathways and the KEGG pathway maps of the enriched pathways in the folder [KEGG_pathways](./gProfileR/KEGG_pathways); in these maps, red corresponds to upregulated and green to downregulated parts of the pathways. There hardly is a difference in colour intensity between ~ 0.5 and 2.5 fold change.
Shown below you find the KEGG pathways enrichment plot:")
# knitr::include_graphics(paste0(wd,path))
# TODO: adjust the pathway above - check what happens with gProfileR if no contrast is provided.
```
