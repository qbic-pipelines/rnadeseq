...
if(file.exists(QAfile)){
  ...
  print(names(df_QA))
  df_QA$Extract.Code.s. <- NULL
  df_QA$Extract.Name.s. <- NULL
  df_QA$Extract.Lab.ID.s. <- NULL
  if ("RIN" %in% colnames(df_QA)){
    if (any(is.na(df_QA$RIN))){no_RIN <- TRUE} else {RIN <- TRUE}
  } else {no_RIN <- TRUE}
} else {
  no_RIN <- TRUE
}
...
is summarized in the table below and can be found [here](./differential_gene_expression/metadata/Sample_preparations.tsv). 
The RNA Integrity Number (RIN) value has a maximum value of 10.
\
")
#datatable(df_QA, caption = "Quality Assessment", extensions = c('Buttons', 'FixedColumns'), 
#          "Autofill", options = list(dom = 'Bfrtip', scrollX = T, scrollCollapse = T, 
...
found [here](./differential_gene_expression/metadata/Sample_preparations.tsv).
\
")
if ("RIN" %in% colnames(df_QA)){df_QA$RIN <- NULL}
#datatable(df_QA, caption = "Sample metadata", extensions = c('Buttons', 'FixedColumns'), 
#          "Autofill", options = list(dom = 'Bfrtip', scrollX = T, scrollCollapse = T, 
#          buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
kable(df_QA) %>%
    kable_styling() %>%
...
## Read Mapping and Assignment Summary
...
metadata <- df_DE <- read.table((paste0(wd, "/differential_gene_expression/metadata/metadata.tsv")), header=TRUE, sep="\t")
...
The closer the distance between two samples is to 0, the more similar the gene expression between these samples is.
...
library(DT)
library(tidyverse)
df_DE <- read.table(paste0(wd,"/differential_gene_expression/final_gene_table/final_DE_gene_list.tsv"), header=TRUE, sep="\t")
...
datatable(df, caption = "Outcome of Differential Expression analysis", extensions = c('Buttons', 'FixedColumns'), 
          "Autofill", options = list(dom = 'Bfrtip', scrollX = T, scrollCollapse = T, 
          buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
```
...
knitr::include_graphics(paste0(wd,"/pathway_analysis/", 'DE_contrast_', contrnames[i], "/", 'DE_contrast_', contrnames[i], '_keg_pathway_enrichment_plot.png'))
```
...
```
\
The results for all work packages, as described in the [quote](`r params$path_quote`) can be found in this report. 
