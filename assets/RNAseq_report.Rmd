...
```{r, out.width="100%", out.height="100%"}
    ...
    pg[['x']][['layout']][['annotations']][[1]][['y']] <- -0.05
     # This adds additional plot export formats to the menubar
    pdf_exp <- list(
        name = "Download plot as a pdf",
        icon = list(
            path = "fileformats/pdf.svg"
        ),
        click = htmlwidgets::JS(
            "function(pg) {
                Plotly.downloadImage(pg, {format: 'pdf', filename: 'volcanoplot'});
            }"
        )
    )
    svg_exp <- list(
        ...
        icon = list(
            path = "fileformats/svg.svg"
        ),
        click = htmlwidgets::JS(
            "function(pg) {
                Plotly.downloadImage(pg, {format: 'svg', filename: 'volcanoplot'});
            }"
        )
    )
    config(pg, modeBarButtonsToAdd = list(pdf_exp, svg_exp), toImageButtonOptions = list(
      format = "pdf"
    )) %>% layout(margin = list(l = 100, b=100), width=700, height = 400*length(contrast_files[grepl("allgenes", contrast_files, fixed = T)]))
```
