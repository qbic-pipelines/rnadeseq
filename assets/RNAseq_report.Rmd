...
write.table(df_DE, file=file.path("/sfs/9/ws/iivow01-test/rnadeseq/mqc_new/error", "df_DE1.tsv"), row.names=F, sep="\t", quote=F)
mqc_version <- ifelse("FastQC..raw._mqc.generalstats.fastqc_raw.total_sequences" %in% colnames(df_DE), "new_mqc", 'old_mqc')
...
write.table(df_DE_selected, file=file.path("/sfs/9/ws/iivow01-test/rnadeseq/mqc_new/error", "df_DE2.tsv"), row.names=F, sep="\t", quote=F)
...
if (mqc_version == 'old_mqc') {
    ...
    colnames = c("Sample", "Number of reads/raw (M)", "Duplicates/raw (%)", "GC/raw (%)", "Number of reads/trimmed (M)", "Duplicates/trimmed (%)", "GC/trimmed (%)", "Trimmed reads (%)", "Mapped reads (%)")
    colnames(table_complete) <- c("Sample", "ReadNumberRaw", "DuplicateReadsInterceptRaw", "GCcontentRaw", "ReadNumberTrimmed", "DuplicateReadsInterceptTrimmed", "GCcontentTrimmed", "TrimmedReads", "MappedReads")
    table_complete <- table_complete %>%
        mutate(
            Sample = Sample,
            ReadNumberRaw = color_bar("lightblue")(round((ReadNumberRaw/1000000),2)),
            DuplicateReadsInterceptRaw = ifelse(DuplicateReadsInterceptRaw > 1,
                                            cell_spec(DuplicateReadsInterceptRaw, color="orange", bold=T),
                                            cell_spec(DuplicateReadsInterceptRaw, color="black")),
            GCcontentRaw = GCcontentRaw,
            ReadNumberTrimmed = color_bar("lightblue")(round((ReadNumberTrimmed/1000000),2)),
            DuplicateReadsInterceptTrimmed = ifelse(DuplicateReadsInterceptTrimmed > 1,
                                            cell_spec(DuplicateReadsInterceptTrimmed, color="orange", bold=T),
                                            cell_spec(DuplicateReadsInterceptTrimmed, color="black")),
            GCcontentTrimmed = GCcontentTrimmed,
            TrimmedReads = TrimmedReads,
            MappedReads =  ifelse(MappedReads > 80,
                                color_bar("lightblue")(MappedReads),
                                color_bar("orange")(MappedReads))
...
write.table(table_complete, file=file.path("/sfs/9/ws/iivow01-test/rnadeseq/mqc_new/error", "table_complete3_before.tsv"), row.names=F, sep="\t", quote=F)

#table_complete <- table_complete %>%
#    dplyr::select(1,8:13)
#write.table(table_complete, file=file.path("/sfs/9/ws/iivow01-test/rnadeseq/mqc_new/error", "table_complete3_after.tsv"), row.names=F, sep="\t", quote=F)
...
The statistics of read alignment to genes are shown below. Most reads should be aligned. Interpretation of the different alignment types:

-   **Uniquely mapped:** The reads could properly be aligned to a gene or feature. \
-   **Mapped to multiple/too many loci:** The reads could be aligned to multiple genes because of its ambiguous character. \
-   **Unmapped: too short/other:** The reads could not be aligned to any gene, e.g. because they were too short. \
")
...
```{r FC_percentages_plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/")), out.width="160%", dpi=1200, fig.cap=ifelse(mqc_version == 'old_mqc', "featureCounts: Assignments", "STAR: Alignments"), fig.align='center'}
if (mqc_version == 'old_mqc') {
...
```{r FC_readnums_plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/")), out.width="160%", dpi=1200, fig.cap=ifelse(mqc_version == 'old_mqc', "featureCounts: Assignments", "STAR: Alignments"), fig.align='center'}
if (mqc_version == 'old_mqc') {
