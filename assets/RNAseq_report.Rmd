...
# If custom GMT file was provided, filter it for upload it to use in gost queries (datasources )
if (isProvided(params$gost_gmt)){

    gost_id <- upload_GMT_file(params$gost_gmt)
} else {
    gost_id <- organism
}
...
for (file in contrast_files){
    ...
    write(q, file=paste0("/home-link/iivow01/git/template_gprofiler_tests/104/", fname))
    # gost query
...
```{r gost, eval=params$pathway_analysis, out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE}
    ...
    capture.output(q_list, file=("/home-link/iivow01/git/template_gprofiler_tests/104/q_list"))
