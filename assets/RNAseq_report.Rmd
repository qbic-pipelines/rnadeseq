...
Pathway analysis is performed with the `gProfiler2` tool. The plot below summarizes the pathways that were 
found significantly deregulated (padj value > 0.05). 
...
```{r, echo=FALSE}
  pathenrich_table <- path_enrich
  pathenrich_table$parents <- NULL
  kable(pathenrich_table) %>%
    kable_styling("hover") %>%
    scroll_box(width = "100%", height = "400px")
```
...
cat(paste0("## KEGG pathway list
Enriched KEGG pathways were found using a pathway analysis with `gProfileR`. The results are stored in [this folder](./pathway_analysis/), 
divided in subfolders, one for each contrast. Each subfolder contains a list of the enriched pathways, stored in the file ending with `results.tsv`, 
and also the plots of the KEGG pathway maps of the enriched pathways, that can be found in the subfolder named `KEGG_pathways`; 
...
knitr::include_graphics(paste0(wd,"/pathway_analysis/", 'DE_contrast_', contrnames[1], "/", 'DE_contrast_', contrnames[1], '_keg_pathway_enrichment_plot.png'))
```
