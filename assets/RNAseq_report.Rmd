...
library(tidyverse)
# system("unzip params$path_multiqc_zip -d params$path_wd")
uzp = params$path_multiqc_zip
exdir = params$path_wd
unzip(uzp, exdir = exdir)
```

## Bioinformatics Analysis Quality Control Summary
Quality control of the data was performed using *FASTQC*. Details on the quality control parameters are available in the `MultiQC` report of all samples available [here](`r paste0(wd,"/multiqc_report.html")`). `FASTQC` reports for individual samples are additionally available [here](`r params$path_fastqc`).
\
...
knitr::include_graphics(paste0(wd, "/multiqc_plots/svg/mqc_star_alignment_plot_1_pc.svg"))
```

#### Number of reads {-}
```{r, echo=F, out.width="160%", dpi=1200, fig.cap="STAR: Alignment Scores", fig.align='center'}
knitr::include_graphics(paste0(wd, "/multiqc_plots/svg/mqc_star_alignment_plot_1.svg"))
```
</center>
\
...
knitr::include_graphics(paste0(wd, "/multiqc_plots/svg/mqc_featureCounts_assignment_plot_1_pc.svg"))
```
</center>
...
<center>
```{r, echo=F, out.width="160%", dpi=1200, fig.cap="featureCounts: Assignments", fig.align='center'}
knitr::include_graphics(paste0(wd, "/multiqc_plots/svg/mqc_featureCounts_assignment_plot_1.svg"))
```
</center>
\
...
knitr::include_graphics(paste0(wd, "/multiqc_plots/svg/mqc_rseqc_read_distribution_plot_1_pc.svg"))
```
</center>
...
<center>
```{r, echo=F, out.width="160%", dpi=1200, fig.cap="RSeQC: Read Distribution", fig.align='center'}
knitr::include_graphics(paste0(wd, "/multiqc_plots/svg/mqc_rseqc_read_distribution_plot_1.svg"))
```
</center>
...
knitr::include_graphics(paste0(wd,"/DESeq2/results/plots/PCA_plot.svg"))
```
...
```{r, echo=F, out.width="2000px", dpi=1200, fig.cap="", fig.align='center'}
knitr::include_graphics(paste0(wd,"/DESeq2/results/plots/Heatmaps_of_distances.svg"))
```
...
library(DT)
library(tidyverse)
df_DE <- read.table(paste0(wd,"/DESeq2/results/final/final_list_DESeq2.tsv"), header=TRUE, sep="\t")
...
cat("***
Additional tables for all individual contrasts are provided [here](`r paste0(wd,"/DESeq2/results//home/silvia/Documents/rnaseq_downstream/r-analysis-templates/report/DESeq2/results/DE_genes_tables/")`:
\
")
...
Plots of the fitted expression values of each exon were generated for a subset of 20 genes among those showing differential exon usage. The plots are available in [this folder](`r paste0(wd,/DESeq2/results/plots/plots_example_genes)`); in each graph both the experimental conditions are indicated, and the exon that showed significant differential usage is shown in purple.")
```
