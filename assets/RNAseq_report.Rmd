...
- `plots/`:
    - `Heatmaps_of_distances.pdf/.svg`: heatmap of the pairwise euclidean distances among samples, when the `r norm_method_text` normalized gene counts are considered.
...
for (file in contrast_files){
    ...
    # Find out which conditions are being compared in the current contrast to filter the heatmaps matrix
    comparison <- unlist(strsplit(fname, "_", fixed=T))
    vs <- which(comparison == "vs") # Get position of 'vs', e.g. DE_contrast_condition_cellline_H1_vs_H2 --> vs = 6
    cond0 <- comparison[vs-2] # Get contrast, e.g. cellline
    cond1 <- comparison[vs-1] # Get first condition, e.g. H1
    cond2 <- comparison[vs+1] # Get second condition, e.g. H2
    ...
    if (nrow(pathway_gostres) > 0){ #if there are enriched pathways
        ...
        for (df in res){
            ...
            ggsave(p, filename = paste0("pathway_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.png"), device = "png", height = 5+0.5*nrow(df_subset), units = "cm", dpi = 300, limitsize=F)
            ggsave(p, filename = paste0("pathway_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.svg"), device = "svg", height = 5+0.5*nrow(df_subset), units = "cm", dpi = 300, limitsize=F)
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    mat$Ensembl_ID <- NULL
                    mat <- data.matrix(mat)
    capture.output(fname, file="/home-link/iivow01/git/rnadeseq/error/fname", append=F)
    capture.output(vs, file="/home-link/iivow01/git/rnadeseq/error/fname", append=T)
    capture.output(cond0, file="/home-link/iivow01/git/rnadeseq/error/fname", append=T)
    capture.output(cond1, file="/home-link/iivow01/git/rnadeseq/error/fname", append=T)
    capture.output(cond2, file="/home-link/iivow01/git/rnadeseq/error/fname", append=T)
    write.table(mat, file="/home-link/iivow01/git/rnadeseq/error/mat.tsv", quote=F, sep="\t")
    write.table(metadata_save, file="/home-link/iivow01/git/rnadeseq/error/metadata_save.tsv", quote=F, sep="\t")

                    # Find metadata column name of the current contrast
                    cond0_keep <- unlist(grep(paste0("(", cond0, ")$"), colnames(metadata_save), value=T))[1]
                    capture.output(grep(paste0("(", cond0, ")$"), colnames(metadata_save), value=T), file="/home-link/iivow01/git/rnadeseq/error/grep", append=F)
                    capture.output(cond0_keep, file="/home-link/iivow01/git/rnadeseq/error/cond0_keep", append=F)
                    # Subset metadata to those samples that have the current conditions in the current contrast, e.g. genotype is H1 or H2
                    metadata_keep <- metadata_save[metadata_save[,cond0_keep] %in% c(cond1,cond2),]
                        write.table(metadata_keep, file="/home-link/iivow01/git/rnadeseq/error/metadata_keep.tsv", quote=F, sep="\t")
# Subset matrix accordingly
                    capture.output(rownames(metadata_keep), file="/home-link/iivow01/git/rnadeseq/error/row_met", append=F)
                    capture.output(colnames(mat), file="/home-link/iivow01/git/rnadeseq/error/cols_mat", append=F)
                    capture.output(typeof(mat), file="/home-link/iivow01/git/rnadeseq/error/type_mat", append=F)

                    mat <- as.data.frame(mat[,rownames(metadata_keep)])
                    capture.output(typeof(mat), file="/home-link/iivow01/git/rnadeseq/error/type_mat", append=T)
                        write.table(mat, file="/home-link/iivow01/git/rnadeseq/error/mat2.tsv", quote=F, sep="\t")
                        ...
                        tryCatch(
                            {
                                ...
                                pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows)
                                dev.off()
                            },
                            error=function(e) {
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        ...
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific heatmap of normalized expression values for genes", scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows)
        dev.off()
