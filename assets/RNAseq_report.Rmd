...
log_FC_text <- as.character(params$log_FC_threshold)
```
...
```{r, out.width="100%", out.height="100%"}
    ...
    log_FC_num <- as.numeric(params$log_FC_threshold)
    # Set plot theme
    ...
    pg <- ggplot(DE_all, aes(x=log2FoldChange, y=logpval, text=paste("Gene: ", gene_name, "<br>", "Log2FC: ", formatC(log2FoldChange, digits=2)))) +
            ...
            geom_point(alpha=0.5, aes(color=ifelse(abs(DE_all$log2FoldChange) > params$log_FC_threshold &  DE_all$logpval > -log10(pval_threshold), "Differentially expressed genes", "Non-differentially expressed genes"))) +
            ...
            geom_vline(xintercept = 0.05, size = 0.2, color = "grey") +
            geom_vline(xintercept = -0.05, size = 0.2, color = "grey") +
...
capture.output(params$min_DEG_pathway, file="/mnt/volume/git/rnadeseq/bla/mindegpar", append=T)
capture.output(typeof(params$min_DEG_pathway), file="/mnt/volume/git/rnadeseq/bla/mindegpar", append=T)
...
capture.output(contrast_files, file="/mnt/volume/git/rnadeseq/bla/contrast_files", append=T)
for (file in contrast_files){
    ...
    capture.output(DE_genes, file="/mnt/volume/git/rnadeseq/bla/de_genes", append=T)
    ...
    capture.output(pathway_gostres, file="/mnt/volume/git/rnadeseq/bla/gostres", append=T)
    ...
    capture.output(pathway_gostres, file="/mnt/volume/git/rnadeseq/bla/gostressig", append=T)
    # Select only pathways with a min. number of DEG
    capture.output(min_DEG_pathway, file="/mnt/volume/git/rnadeseq/bla/mindeg", append=T)
    pathway_gostres <- pathway_gostres[which(pathway_gostres$intersection_size>=min_DEG_pathway),]
    capture.output(pathway_gostres$intersection_size, file="/mnt/volume/git/rnadeseq/bla/intersize", append=T)
    gostres$result <- pathway_gostres
    capture.output(pathway_gostres, file="/mnt/volume/git/rnadeseq/bla/gostresdeg", append=T)
    # Plot pathways if there were any
    capture.output(nrow(pathway_gostres), file="/mnt/volume/git/rnadeseq/bla/nrowgostres", append=T)
    ...
    if (nrow(pathway_gostres) > 0){ #if there are enriched pathways
        ...
        print("Plotting heatmaps...")
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                capture.output(conditions, file="/mnt/volume/git/rnadeseq/bla/conditions", append=F)
                capture.output(metadata_cond, file="/mnt/volume/git/rnadeseq/bla/metadata_cond", append=F)
                capture.output(metadata_name, file="/mnt/volume/git/rnadeseq/bla/metadata_name", append=F)
                row.names(metadata_cond) <- apply(metadata_name,1,paste, collapse = "_")

                for (i in c(1:nrow(df))){
                pathway <- df[i,]
                gene_list <- unlist(strsplit(pathway$intersection, ","))
                capture.output(pathway$intersection, file="/mnt/volume/git/rnadeseq/bla/pathway$intersection", append=F)
                capture.output(gene_list, file="/mnt/volume/git/rnadeseq/bla/gene_list", append=F)
                mat <- norm_counts[gene_list, ]
                capture.output(norm_counts, file="/mnt/volume/git/rnadeseq/bla/norm_counts", append=F)
                capture.output(mat, file="/mnt/volume/git/rnadeseq/bla/mat", append=F)
                capture.output(mat$gene_name, file="/mnt/volume/git/rnadeseq/bla/mat_genena", append=F)
                rownames(mat) <- mat$gene_name
                capture.output("schalala", file="/mnt/volume/git/rnadeseq/bla/schalala")
                mat$gene_name <- NULL
                mat <- data.matrix(mat)
                capture.output(mat, file="/mnt/volume/git/rnadeseq/bla/mat2")
                capture.output(nrow(mat), file="/mnt/volume/git/rnadeseq/bla/nrowmat", append=T)
                if (nrow(mat)>1){
