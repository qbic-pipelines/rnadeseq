...
This visualizes how similar the samples are to each other according to the Euclidean distance of gene expression.
Two samples have more similar gene expression if their distance is closer to 0 (darker blue).
The original plot can be downloaded [here](./differential_gene_expression/plots/Heatmaps_of_distances_euclidean.pdf).
...
par(oma=c(3,3,3,3))
write.table(sampleDistMatrix, file="differential_gene_expression/plots/Heatmaps_of_distances_euclidean.tsv", sep="\t", quote=F)
dist_heatmap <- pheatmap(mat = sampleDistMatrix, color=colors, cluster_rows = sampleDistClust, cluster_cols = sampleDistClust)

svg("differential_gene_expression/plots/Heatmaps_of_distances_euclidean.svg")
dist_heatmap
dev.off()
png("differential_gene_expression/plots/Heatmaps_of_distances_euclidean.png")
dist_heatmap
dev.off()
pdf("differential_gene_expression/plots/Heatmaps_of_distances_euclidean.pdf")
dist_heatmap
dev.off()
...
- `plots/`:
    - `Heatmaps_of_distances_euclidean.pdf/.svg`: heatmap of the pairwise euclidean distances among samples, when the `r norm_method_text` normalized gene counts are considered.
...
capture.output(pathway_gostres, file=paste0("pathway_analysis", "/", fname, "/pathway_gostres.tsv"))
        # Splitting results according to pathway resources (KEGG / REACTOME / GO)
        res <- split(pathway_gostres, pathway_gostres$source)
        for (df in res){
            capture.output(df, file=paste0("pathway_analysis", "/", fname, "/df.tsv"))
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        write.table(norm_counts, file=paste0("pathway_analysis", "/",fname, "/", "pathway_heatmaps", "/", "norm_counts", pathway$source, "_", make.names(pathway$term_id), ".tsv"), sep="\t", quote=F, row.names=F)
...
if (isProvided(params$path_genelist)){
    ...
    rownames(norm_counts) <- norm_counts$Ensembl_ID
    mat <- norm_counts[which(norm_counts$Ensembl_ID %in% gene_list), ]
    write.table(norm_counts, file=paste0(genelist_heatmaps_dir, "/", "norm_counts.tsv"), sep="\t", quote=F, row.names=T)

    if (nrow(mat)>1){
        write.table(mat, file=paste0(genelist_heatmaps_dir, "/", "mat.tsv"), sep="\t", quote=F, row.names=F)
