...
pca <- ggplot(pcaData, aes(PC1, PC2, color=combfactor, text=paste("PC1: ", PC1, "<br>", "PC2: ", PC2))) +
            geom_point(size=3) +
...
percentVar <- round(100*attr(pcaData, "percentVar"))
pca2 <- ggplot(pcaData2, aes(PC1, PC2, color=combfactor, text=paste("PC1: ", PC1, "<br>", "PC2: ", PC2))) +
            geom_point(size=3)+
            xlab(paste0("PC1: ", percentVar[1],"% variance")) +
            ylab(paste0("PC2: ", percentVar[2], "% variance")) +
            coord_fixed() +
...
```{r remove_batch, include=F}
#Remove batch columns (not condition_batch columns) from cds as they will otherwise lead to problems with DE
for (c in colnames(colData(cds))) {
  if (startsWith(c, "batch")) {
    colData(cds)[,c] <- NULL
  }
}
```
...
coefficients <- coefficients[!startsWith(coefficients, "batch_")]
...
if (isProvided(params$path_contrast_matrix)){
    capture.output(cds, file="/home-link/iivow01/git/rnadeseq/error/cds1")
    ...
    capture.output(contrasts, file="/home-link/iivow01/git/rnadeseq/error/cont1")
    contrasts <- contrasts[!(startsWith(row.names(contrasts), "batch_")),]
    capture.output(contrasts, file="/home-link/iivow01/git/rnadeseq/error/cont2")
    capture.output("cols", file="/home-link/iivow01/git/rnadeseq/error/cont2", append=T)
    for (c in colnames(contrasts)) {
        capture.output(c, file="/home-link/iivow01/git/rnadeseq/error/cont2", append=T)
    }
    capture.output("rows", file="/home-link/iivow01/git/rnadeseq/error/cont2", append=T)
    for (c in rownames(contrasts)) {
        capture.output(c, file="/home-link/iivow01/git/rnadeseq/error/cont2", append=T)
    }
    capture.output(coefficients, file="/home-link/iivow01/git/rnadeseq/error/coeffs")
    capture.output("cols", file="/home-link/iivow01/git/rnadeseq/error/coeffs", append=T)
    for (c in colnames(coefficients)) {
        capture.output(c, file="/home-link/iivow01/git/rnadeseq/error/coeffs", append=T)
    }
    capture.output("rows", file="/home-link/iivow01/git/rnadeseq/error/coeffs", append=T)
    for (c in rownames(coefficients)) {
        capture.output(c, file="/home-link/iivow01/git/rnadeseq/error/coeffs", append=T)
    }
    aslidhasild
    ...
    capture.output(contrasts, file="/home-link/iivow01/git/rnadeseq/error/contrasts")
    capture.output(resultsNames(cds), file="/home-link/iivow01/git/rnadeseq/error/cds2")
    capture.output(
    colnames(colData(cds)), file="/home-link/iivow01/git/rnadeseq/error/colnames")
    for (c in colnames(colData(cds))) {
        if (startsWith(c, "batch_")) {
            colData(cds)[,c] <- NULL
        }
    }

    capture.output(resultsNames(cds), file="/home-link/iivow01/git/rnadeseq/error/cds3")
...
```{r volcano_plot, out.width="100%", out.height="100%"}
    ...
    pg <- ggplot(DE_all, aes(x=log2FoldChange, y=logpval, text=paste("Gene: ", gene_name, "<br>", "Log2FC: ", formatC(log2FoldChange, digits=2)))) +
            ...
            ylab("-log 10 padj") + xlab("Log2 Fold Change") + theme(panel.spacing = unit(ifelse(length(allgenes_files)>2, 0.1, 0.6), "lines"))
    ...
    #add export button to plot
    config(pg, modeBarButtonsToAdd = list(svg_exp)) %>% layout(margin = list(b = ifelse(length(allgenes_files)>2, 150, 100)), width=700, height = 500*length(allgenes_files))
```
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste(pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T )
                        dev.off()
                    ...
                    if (pathway$source == "KEGG"){
                        ...
                        tryCatch({
                            ...
                            entrez_ids = mapIds(species_library_installed, keys=as.character(gene.data.subset$Ensembl_ID), column = "ENTREZID", keytype=params$keytype, multiVals="first")
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        ...
        pheatmap(mat = mat, annotation_col = metadata_save, main = "", scale = "row", cluster_cols = F, cluster_rows = T )
        dev.off()
...
```{r gost, eval=params$pathway_analysis, out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE}
    ...
    if (nrow(path_enrich) > 0){
        ...
        #add export button to plot
        config(pg2, modeBarButtonsToAdd = list(svg_exp)) %>% layout(margin = list(l = 75, b = 100), width=500, height = 400*length(contrast_files))
    } else {
...
version <- read.csv(file = params$path_software_versions, sep="\t", header = FALSE)
```
...
database_string <- paste0(substr(database_string,1,nchar(database_string)-2), ".")
cat(paste0("For pathway analysis, the R packages `gprofiler2 v", version_gprofiler2," `, `AnnotationDbi v", version_annotation, "` and `", name_species, " v", version_annotation, "` were employed. Also, the following databases were queried: ", database_string))
```
