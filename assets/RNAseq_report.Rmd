...
vst_cutoff <- 0.005
var_sizeFactors <- var(sizeFactors(cds))
run_rlog <- T
...
if (params$trafo == "vst-force") {
    run_rlog <- F
    trafo_long_text <- "variant stabilizing transformation (vst)"
    nsub_genes_text <- paste0("The number of subset genes for the vst transformation is ", as.character(params$nsub_genes), ".")
...
For PCA analysis and heatmap plotting, the `r trafo_long_text` normalized gene counts were employed. 
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        png(filename = paste("pathway_analysis", "/",fname, "/", "pathway_heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", pathway$term_id, "_",fname, ".png", sep=""), width = ifelse(nrow(mat)<=20, 200, 125)+ncol(mat)*40, height = 50+nrow(mat)*50)
                        ...
                        dev.off()
                        pdf(paste("pathway_analysis", "/", fname, "/", "pathway_heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", pathway$term_id, "_", fname, ".pdf", sep=""), width = ifelse(nrow(mat)<=20, 50, 10)+ncol(mat)*0.1, height = 5+nrow(mat)*0.005)
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T)
                        dev.off()
                        svg(paste("pathway_analysis", "/", fname, "/", "pathway_heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", pathway$term_id, "_", fname, ".svg", sep=""), width = ifelse(nrow(mat)<=20, 50, 10)+ncol(mat)*0.1, height = 5+nrow(mat)*0.005)
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T)
                        dev.off()
                    }
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        png(filename = paste("pathway_analysis", "/", genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.png", sep=""), width = 100+ncol(mat)*50, height = 50+nrow(mat)*100)
        ...
        dev.off()
        pdf(paste("pathway_analysis", "/", genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.pdf", sep=""), width = 10+ncol(mat)*0.05, height = 5+nrow(mat)*0.005)
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific heatmap of normalized expression values for genes", scale = "row", cluster_cols = F, cluster_rows = T)
        dev.off()
        svg(paste("pathway_analysis", "/", genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.svg", sep=""), width = 10+ncol(mat)*0.1, height = 5+nrow(mat)*0.005)
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific heatmap of normalized expression values for genes", scale = "row", cluster_cols = F, cluster_rows = T)
        dev.off()
    }
}
...
```{r conditional_pathway, echo=FALSE, results='asis', eval=params$pathway_analysis&isProvided(params$path_software_versions)}
...
if (isProvided(params$custom_gmt)){
    ...
    database_string <- paste0(database_string, ".") # Add final dot to sentence
}

cat(paste0(
"For pathway analysis, the R packages `gprofiler2 v",
version_gprofiler2,
" `, `AnnotationDbi v",
version_annotation,
"` and `",
