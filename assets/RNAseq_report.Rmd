...
```{r pathway_analysis, echo=FALSE, results='asis', eval=report_options$KEGG & params$pathway_analysis != ""}
cat(paste0("***
# Pathway analysis
...
```{r, eval=report_options$KEGG & params$pathway_analysis != "", out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE}
    library(gprofiler2)
    #library(rlist)
    ...
    # Subsetting to pathways with minimum number of DEG
    if(!is.na(as.integer(params$path_min_DEG))) {
        path_enrich <- path_enrich[which(path_enrich$intersection_size>=params$path_min_DEG),]
    }
    gostres$result <- path_enrich
...
```{r, eval=report_options$KEGG & params$pathway_analysis != "", echo=FALSE}
if (nrow(path_enrich) > 0) {
...
```{r conditional_KEGG_contrasts, echo=FALSE, results='asis', eval=report_options$KEGG & params$pathway_analysis != ""}
...
```{r conditional_pathway_maps, echo=FALSE, results='asis', eval=report_options$KEGG & params$pathway_analysis != ""}
