...
if (length(q_list) > 0) {
    ...
    if (nrow(path_enrich) > 0){
        ...
        for (counter in c(1:length(contrast_files))) {
            expression <- parse(text=paste0("pg2 %>% layout(yaxis", ifelse(counter>1, counter, "")," = list(range = list(0, ceiling(2*-log10(max(gostres$p_value))))))"))
            pg2 <- eval(expression)
            pg2 %>% layout(yaxis2 = list(range = list(0, ceiling(2*-log10(max(gostres$p_value))))))
        }
