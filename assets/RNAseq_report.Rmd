...
build_smrnaseq_df <- function(files) {
    ...
    for (f in files) {
        ...
        # If merged_mat already exists, add third hairpin column to it (second column after rownames)
        if (is.data.frame(merged_mat)) {
            ...
            # Otherwise initiate it from third hairpin column and rownames
...
if (params$input_type == "smrnaseq") {
    ...
    hairpin_files <- file.path(gsub("/$", "", params$path_gene_counts), "hairpin", paste0(qbicCodes, "_mature_hairpin.sorted.bam.idxstats"))
    if (!(all(file.exists(hairpin_files)))) {
        missing_files <- "Could not find all of the specified <QBiC-code>_mature_hairpin.sorted.bam.idxstats files! Please check that for all QBiC codes in the metadata sheet, a <QBiC-code>_mature_hairpin.sorted.bam.idxstats file is present in the gene_counts folder and make sure that no typos exist. The following file(s) could not be found: "
    ...
    # Non-hairpins next
    other_files <- file.path(gsub("/$", "", params$path_gene_counts), "mature", paste0(qbicCodes, "_mature.sorted.bam.idxstats"))
    if (!(all(file.exists(other_files)))) {
        missing_files <- "Could not find all of the specified <QBiC-code>_mature.sorted.bam.idxstats files! Please check that for all QBiC codes in the metadata sheet, a <QBiC-code>_mature.sorted.bam.idxstats file is present in the gene_counts folder and make sure that no typos exist. The following file(s) could not be found: "
...
if (isProvided(params$path_contrast_matrix)){
    ...
    for (i in c(1:ncol(contrasts))) {
        ...
        DE_genes_contrast <- DE_genes_contrast_genename #subset(DE_genes_contrast_genename, padj <= params$adj_pval_threshold & abs(log2FoldChange) >= params$logFC_threshold)
...
write(params$pathway_analysis, file="/sfs/9/ws/iivow01-test/rnadeseq/error/qw")

```

```{r gost, eval=params$pathway_analysis, out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE}
write(params$pathway_analysis, file="/sfs/9/ws/iivow01-test/rnadeseq/error/werwe")
...
if (length(q_list) > 0) {
    ...
    path_enrich=data.frame(list())
    write("lahdladsh", file="/sfs/9/ws/iivow01-test/rnadeseq/error/eee")
