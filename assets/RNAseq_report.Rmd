...
## Load Rsem count table
    if (params$input_type == "rsem") {
        ...
        if (!(all(file.exists(files)))) {
            missing_files <- "DESeq2.R could not find all of the specified .genes.results files! Please check that for all QBiC codes in the metadata sheet, a quant.sf file is present in the gene_counts folder and make sure that no typos exist. The following file(s) could not be found: "
            for (f in files) {
                if (!file.exists(f)) {
