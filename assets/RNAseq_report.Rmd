...
df <- read.csv(file=paste0(wd, params$path_qc_summary), sep="\t", header = TRUE)
...
Quality control of the data was performed using *FASTQC*. Details on the quality control parameters are available in the `MultiQC` report of all samples available [here](paste0(wd, "/multiqc_report.html)). `FASTQC` reports for individual samples are additionally available [here](`r params$path_fastqc`).
\
