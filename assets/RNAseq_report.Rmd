...
if (!(isNot(params$path_contrast_list))) {
    ...
    write.table(metadata_cond, file="/home-link/iivow01/git/rnadeseq/error/metadata_cond", append=F)
    for (i in 1:nrow(contrasts)) {
        ...
        if (grepl("condition_", contrasts[i,1], fixed = TRUE)) {
            ...
            capture.output(cond_name, file="/home-link/iivow01/git/rnadeseq/error/metcond", append=T)
            capture.output(metadata_cond[cond_name], file="/home-link/iivow01/git/rnadeseq/error/metcond", append=T)
            for (j in 2:ncol(contrasts)) {
                if (contrasts[i,j] %in% metadata_cond$cond_name)) {
                    capture.output(paste(contrasts[i,j], "is in"), file="/home-link/iivow01/git/rnadeseq/error/contr", append=T)
                    capture.output(metadata_cond$cond_name, file="/home-link/iivow01/git/rnadeseq/error/contr", append=T)
                } else {
                    capture.output(paste(contrasts[i,j], "is not in"), file="/home-link/iivow01/git/rnadeseq/error/contr", append=T)
                    capture.output(metadata_cond$cond_name, file="/home-link/iivow01/git/rnadeseq/error/contr", append=T)
                }
            }
        }
