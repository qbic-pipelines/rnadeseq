...
```{r docSetup, bootstrap.show.code = FALSE, dev = c('png', 'svg'), bootstrap.show.message=FALSE, echo=FALSE, message=FALSE}
## knitrBoostrap and device chunk options
options(warn = -1)                          # suppress warnings and messages
library(knitr)
#TODO: Check if this works
opts_chunk$set(bootstrap.show.code = FALSE, dev = c('png', 'svg'), echo=FALSE)
```
...
```{r, out.width="100%", out.height="100%"}
    ...
    for (file in contrast_files){
        if (grepl("allgenes", file, fixed=TRUE))
        {
    ...
    pval_threshold <- 0.05  #set here the not logarithmized threshold for the p value (probably 0.05)
    ...
    pg <- ggplot(DE_all, aes(x=log2FoldChange, y=logpval, text=paste("Gene: ", gene_name, "<br>", "Log2FC: ", formatC(log2FoldChange, digits=2)))) +
            geom_point(aes(alpha=0.3), color=ifelse(abs(DE_all$log2FoldChange) > params$log_FC &  DE_all$logpval > -log10(pval_threshold), "red", "black")) +
            ...
            scale_x_continuous(limits = c(floor(log2FoldChange_min),ceiling(log2FoldChange_max)), breaks = c(floor(log2FoldChange_min):ceiling(log2FoldChange_max))) +
    ...
    pg %>% layout(margin = list(l = 100, b=100), width=500, height = 400*length(contrast_files[grepl("allgenes", contrast_files, fixed=TRUE)]))
```
