...
The linear model employed to model gene expression in DESeq2 
was:
```r cat(paste(readLines(params$path_design), collapse=" "))```
...
# Take the last column on this list, ordering dataframe according to ascending padj, subset fist 20 rows.
...
```{r}
  ...
  for (file in contrast_files){
    ...
    if (nrow(path_enrich) > 0){
      ...
      #pg_list <- list.append(pg_list,pg)
    }
  }
  #htmltools::tagList(list(pg_list))
  pg
```
