...
for (r in 2:nrow(df_DE_selected)) {

    # Iterate over rows
    if (endsWith(df_DE_selected$Sample[r], "-fw")) {
        current_sample_name <- df_DE_selected$Sample[r]
        if (df_DE_selected$Sample[r-1] != substring(current_sample_name, 1, nchar(df_DE_selected$Sample[r])-3)) {

            # For this -fw entry, no entry was found that did not contain the -fw suffix
            stop(paste("Could not find entry for sample", df_DE_selected$Sample[r], "without -fw in next line; quitting..."))
        } else {
            for (c in 1:length(df_DE_selected[r,])) {

                # Iterate over columns in the two relevant rows
                if (is.na(df_DE_selected[r-1,c])) {

                    # Check if current column is empty/NA in the previous row (i.e. the one without -fw)
                    if (is.na(df_DE_selected[r,c])) {

                        # Check if current column is empty/NA in the current row (i.e. the one with -fw) -->if so, no value can be assigned, so stop the script
                        stop(paste("Could not find entry for sample", df_DE_selected$Sample[r], ", value", colnames(df_DE_selected)[c], "; both -fw and non-fw row contain NA; quitting..."))
                    } else {
                        df_DE_selected[r-1,c] <- df_DE_selected[r,c]
                    }
                }
            }
        }
...
# Remove all -fw rows
df_DE_selected <- subset(df_DE_selected, !(endsWith(Sample, "-fw")))
...
if (isProvided(params$path_contrast_pairs)) {
    ...
    #TODO: Do I need to watch out for batch effect here as well?
