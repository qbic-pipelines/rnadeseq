...
cat(config$outlook)
quote_file = list.files(path = wd, pattern = "//.pdf$")
cat("The results for all work packages, as described in the [quote](`r paste0(wd, quote_file)`) can be found in this report. Further support for this project will be restricted to the results presented in this report (e.g. requests to update/manipulate figures and tables). For further analysis (e.g. the re-analysis of the dataset) we will generate a new quote containing cost estimates.")
```
