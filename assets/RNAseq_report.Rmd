...
```{r gost, eval=params$pathway_analysis, out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE}
    capture.output(paste0("häää? ", list.files(path=paste0("differential_gene_expression/DE_genes_tables/"))),
        file="/home-link/iivow01/git/rnadeseq/error/wat", append=T)
    ...
    for (file in contrast_files){
        ...
        #Reading DE genes list
        fname <- tools::file_path_sans_ext(basename(file))
        capture.output(paste0("Reading differential_gene_expression/DE_genes_tables/", file), file="/home-link/iivow01/git/rnadeseq/error/files", append=T)
        DE_genes <- read.csv(file = paste0("differential_gene_expression/DE_genes_tables/", file), sep="\t", header = T)
        capture.output(paste0("Successfully read differential_gene_expression/DE_genes_tables/", file), file="/home-link/iivow01/git/rnadeseq/error/files", append=T)
        ...
        capture.output("Appended successfully", file="/home-link/iivow01/git/rnadeseq/error/files", append=T)
    ...
    if (length(q_list) > 0) {
        ...
        if (nrow(path_enrich) > 0){
            ...
            capture.output("bla1", file="/home-link/iivow01/git/rnadeseq/error/files", append=T)
            ...
            capture.output("bla2", file="/home-link/iivow01/git/rnadeseq/error/files", append=T)
