            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste(pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)))
                        dev.off()
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        ...
        pheatmap(mat = mat, annotation_col = metadata_save, main = "", scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)))
        dev.off()
