...
The Nextflow-based nf-core pipeline `rnaseq `r as.character(version[versions$V1=="nf-core/rnaseq",2])`` [^1] was used for the RNAseq Bioinfomatics analysis. 
...
The differential expression analysis was performed by the use of the `rnadeseq `r as.character(params$revision)``. 
...
```
Graphs were also produced in the `RStudio v1.1.456` with `r R.version.string` mainly using the `R` package `ggplot2 v`r packageVersion("ggplot2")``. 
