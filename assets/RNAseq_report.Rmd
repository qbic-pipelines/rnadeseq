...
if(file.exists(QAfile)){
  ...
  df_QA_cond <- df_QA[ ,conditions]
  df_QA_neat <- df_QA[,c("QBiC Code", "Secondary Name", "Lab ID", "Sample Type")]
  df_QA_neat <- merge(x=df_QA_neat, y=df_QA_cond, by.x="row.names", by.y="row.names")
...
It can also be downloaded [here](./QC/qc_summary.tsv).
...
colnames = c("Sample", "Lab ID", "Number of reads (M)", "Duplicate Intercept (%)", "GC (%)", "Trimmed reads (%)", "Mapped reads (%)", "Assigned reads (%)")
write.table(table_complete,"./qc_summary.tsv", append = FALSE, quote = FALSE, sep = "\t",eol = "\n", na = "NA", dec = ".", row.names = F, col.names=colnames, qmethod = c("escape", "double"))
table_complete %>% 
  mutate(
    Sample = Sample,
    LabID = Lab.ID,
    ReadNumber = color_bar("lightblue")(round((FastQC_mqc.generalstats.fastqc.total_sequences/1000000),2)),
    DuplicateReadsIntercept = ifelse(DupRadar_mqc.generalstats.dupradar.dupRadar_intercept > 1,
                                    cell_spec(DupRadar_mqc.generalstats.dupradar.dupRadar_intercept, color="orange", bold=T),
                                    cell_spec(DupRadar_mqc.generalstats.dupradar.dupRadar_intercept, color="black")),
    GCcontent = FastQC_mqc.generalstats.fastqc.percent_gc,
    TrimmedReads = Cutadapt_mqc.generalstats.cutadapt.percent_trimmed,
    MappedReads =  ifelse(STAR_mqc.generalstats.star.uniquely_mapped_percent > 80,
                          color_bar("lightblue")(STAR_mqc.generalstats.star.uniquely_mapped_percent),
                          color_bar("orange")(STAR_mqc.generalstats.star.uniquely_mapped_percent)),
    AssignedReads = ifelse(featureCounts_mqc.generalstats.featurecounts.percent_assigned > 60,
                          color_bar("lightblue")(featureCounts_mqc.generalstats.featurecounts.percent_assigned),
                          color_bar("orange")(featureCounts_mqc.generalstats.featurecounts.percent_assigned))
  ) %>%
  select(1,9:15) %>%
...
```{r, echo=F, out.width="70%", dpi=1200, fig.align='center'}
...
```{r}
  ...
  for (file in contrast_files){
    ...
    # x: log2FoldChange
    # y: -log10(padj)
    #list.append(table_list, DE_genes)
  }
  #DE_all <- ldply(data, rbind)
  DE_all <- DE_genes
...
## Barplots of gene normalized counts
...
cat(paste0("***
# Pathway analysis
Pathway analysis is performed with the `gProfiler2` tool. The pathway analysis results are stored [here](./pathway_analysis).
...
```{r, eval=report_options$KEGG}
  ...
  for (file in contrast_files){
    ...
    #q_list <- list.append(q_list, q)
  }
  #contrast_names <- sapply(strsub)
  #q_list <- list.append(q_list, q)
  # TODO: add different contrasts in list and make gost plot for all of them
  ...
  path_enrich <- path_enrich[which(path_enrich$significant==TRUE),]

  if (nrow(path_enrich) > 0){
    path_enrich$original.query.size <- rep(length(q), nrow(path_enrich))

    pg <- gostplot(gostres, capped=T, interactive=T)
  
  }
  pg[['x']][['layout']][['annotations']][[1]][['x']] <- -0.05
  #print(pg[['x']][['layout']][['annotations']])
  pg %>% layout(margin = list(l = 75))
```

```{r, echo=FALSE}
  df_subset <- data.frame(Pathway_name = path_enrich$term_name, Pathway_code = path_enrich$term_id, 
  ...
  colnames <- c("Pathway name", "Pathway code", "DE genes in pathway (N)", "Total genes in pathway (N)", 
...
in these maps, red corresponds to upregulated and green to downregulated parts of the pathways.
...
The Nextflow-based nf-core RNA-Seq pipeline release 1.3 https://github.com/nf-core/rnaseq [^1] was used for the bioinfomatics analysis. 
...
control to the human genome GRCh 38 (Ensembl release 92). 
