...
```
```{r conditional_pathway, echo=FALSE, eval=report_options$KEGG}
library(gprofiler2)
library(AnnotationDbi)
```

```{r conditional_pathway, echo=FALSE, results='asis', eval=report_options$KEGG}
...
Graphs were produced in the `RStudio v1.1.456` with `r R.version.string` mainly using the `R` package `ggplot2 v`r packageVersion("ggplot2")``. 
...
[^5]: https://github.com/qbicsoftware/rnadeseq
