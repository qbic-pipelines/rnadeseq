...
df_DE_selected$Sample <- gsub("-fw", "", df_DE_selected$Sample) # Remove -fw to produce duplicate rows for summarising
...
df_DE_selected$Sample <- sub("_[0-9]+$", "", df_DE_selected$Sample) # Remove _1 etc. to produce duplicate rows for summarising
df_DE_selected <- df_DE_selected %>% group_by(Sample) %>% summarise_all(avg_NA)
...
if (mqc_version == 'old_mqc') {
    ...
    colnames = c("Sample", "Number of reads (M)", "Duplicates (%)", "GC (%)", "Number of reads (M)", "Duplicates (%)", "GC (%)", "Trimmed basepairs (%)", "Mapped reads (%)")
    colnames(table_complete) <- c("Sample", "ReadNumberRaw", "DuplicateReadsInterceptRaw", "GCcontentRaw", "ReadNumberTrimmed", "DuplicateReadsInterceptTrimmed", "GCcontentTrimmed", "TrimmedBP", "MappedReads")
    table_complete <- table_complete %>%
        mutate(
            Sample = Sample,
...
if (mqc_version == 'new_mqc') {
    table_complete <- table_complete %>%
        add_header_above(c(" " = 1, "raw" = 3, "trimmed" = 3, " " = 2))   # For new MQC, add header columns raw and trimmed
}
table_complete %>%
    kable_styling("hover") %>%
    scroll_box(width = "100%", height = "400px")
