...
params:
    ...
    #old params, some also for DESeq2
    path_proj_summary: ''
    path_versions: ''
    path_design: ''
    path_wd: ''
...
```{r load, echo=FALSE, bootstrap.show.code=FALSE, message=FALSE, warning=FALSE}
library(dplyr)
library(kableExtra)
library(knitr)
```
...
if (params$rlog){
    ...
    # vst transformation
    # TODO: check if nsub works!
    vsd <- vst(cds, blind=FALSE, nsub = opt$nsub_genes)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/")), results = 'asis'}
cat(paste0("***
...
table_complete %>%
    ...
    kable_styling("hover") %>%
    scroll_box(width = "100%", height = "400px")
    #4 lines above, dplyr:: needs to be specified or an error is thrown: unable to find an inherited method for function 'select' for signature '"data.frame"
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/")), out.width="160%", dpi=1800, fig.cap="STAR: Mapping Statistics", fig.align='center'}
knitr::include_graphics(paste0(wd, "/QC/multiqc_plots/svg/mqc_star_alignment_plot_1_pc.svg"))
```

```{r,echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/"))}
cat(paste0("***
</center>
### Number of reads {-}
<center>
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/")), out.width="160%", dpi=1200, fig.cap="featureCounts: Assignments", fig.align='center'}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=dir.exists(paste0(wd,"/QC/multiqc_data/")), out.width="160%", dpi=1200, fig.cap="RSeQC: Read Distribution", fig.align='center'}
...
suppressMessages(ggsave(plot = pca, filename = "differential_gene_expression/plots/PCA_plot.pdf", device = "pdf", dpi = 300))
suppressMessages(ggsave(plot = pca, filename = "differential_gene_expression/plots/PCA_plot.svg", device = "svg", dpi = 150))
suppressMessages(ggsave(plot = pca, filename = "differential_gene_expression/plots/PCA_plot.png", device = "png", dpi = 150))
########################### PCA PLOT with batch-corrected data ############
if(!isNot(params$batchEffect)){
    if (opt$rlog) {
    ...
    pca2 <- ggplot(pcaData2, aes(PC1, PC2, color=combfactor)) +
                ...
                theme(legend.title = element_blank()) +
                coord_fixed()
    suppressMessages(ggsave(plot = pca2, filename = "differential_gene_expression/plots/PCA_batch_corrected_plot.pdf", device = "pdf", dpi=300))
    suppressMessages(ggsave(plot = pca2, filename = "differential_gene_expression/plots/PCA_batch_corrected_plot.svg", device = "svg", dpi = 150))
    suppressMessages(ggsave(plot = pca2, filename = "differential_gene_expression/plots/PCA_batch_corrected_plot.png", device = "png", dpi = 150))
}

```{r, echo=F, out.width="80%", dpi=1200, fig.align='center'}
#TODO: Embed this directly from object (and following plots)
pca
```
...
```{r, eval=params$pathway_analysis, echo=FALSE, message=FALSE, warning=FALSE, results = 'hide'}
...
if (!(isNot(params$path_contrast_matrix))){
    ...
    for (i in c(1:ncol(contrasts))) {
        ...
        DE_genes_contrast <- subset(DE_genes_contrast_genename, padj < 0.05 & abs(log2FoldChange) > params$log_FC_threshold)
...
if (!(isNot(params$path_contrast_list))) {
    contrasts <- read.table(path_contrast_list, sep="\t", header=T, colClasses = "character")
...
if (isNot(params$path_contrast_matrix) & isNot(params$path_contrast_list) & isNot(params$path_contrast_pairs)) {
    ...
    for (contname in contrast_names) {
        capture.output("", file="/mnt/volume/git/rnadeseq/bug/contnames", append=F)
        capture.output(contname, file="/mnt/volume/git/rnadeseq/bug/contnames", append=T)
        ...
        DE_genes_contrast_genename <- results_DEseq_contrast
        capture.output(DE_genes_contrast_genename, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast_genename1", append=F)
        DE_genes_contrast_genename$Ensembl_ID = row.names(results_DEseq_contrast)
        DE_genes_contrast_genename <- merge(x=DE_genes_contrast_genename, y=gene_names, by.x ="Ensembl_ID", by.y="Ensembl_ID", all.x=T)
        capture.output(DE_genes_contrast_genename, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast_genename2", append=F)
        DE_genes_contrast_genename = DE_genes_contrast_genename[,c(dim(DE_genes_contrast_genename)[2],1:dim(DE_genes_contrast_genename)[2]-1)]
        capture.output(DE_genes_contrast_genename, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast_genename3", append=F)
        DE_genes_contrast_genename = DE_genes_contrast_genename[order(DE_genes_contrast_genename[,"Ensembl_ID"]),]
        capture.output(DE_genes_contrast_genename, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast_genename4", append=F)
        capture.output(DE_genes_contrast_genename$Ensembl_ID, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast_genename$Ensembl_ID", append=F)
        # Save all DE genes (even if not significant) to separate files for the volcano plot
        write.table(DE_genes_contrast_genename, file=paste("differential_gene_expression/allgenes/DE_contrast_",contname,".tsv",sep=""), sep="\t", quote=F, col.names = T, row.names = F)
        # Select only significantly DE
        capture.output(DE_genes_contrast_genename$padj, file="/mnt/volume/git/rnadeseq/bug/padj", append=F)
        pups <- subset(DE_genes_contrast_genename, padj < 0.05)
        pups <- pups$padj
        capture.output("pups11", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(pups, file="/mnt/volume/git/rnadeseq/bug/padjf", append=F)
        pups2 <- subset(DE_genes_contrast_genename, abs(log2FoldChange) > params$log_FC_threshold)
        capture.output("pups21", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        pups2 <- pups2$log2FoldChange
        capture.output("pups22", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(pups2, file="/mnt/volume/git/rnadeseq/bug/logf", append=F)
        pups3 <- subset(DE_genes_contrast_genename, padj < 0.05 & abs(log2FoldChange) > params$log_FC_threshold)
        capture.output(params$log2FoldChange, file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output("median", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(median(pups2), file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output("typeof", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(typeof(pups2), file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(pups2[1][1], file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(typeof(pups2[1][1]), file="/mnt/volume/git/rnadeseq/bug/status", append=T)

        capture.output(length(pups2), file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output("length", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output("pups31", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        pups3 <- pups3["padj","log2FoldChange",drop=F]
        capture.output("pups32", file="/mnt/volume/git/rnadeseq/bug/status", append=T)
        capture.output(pups3, file="/mnt/volume/git/rnadeseq/bug/dropf", append=F)

        DE_genes_contrast <- subset(DE_genes_contrast_genename, padj < 0.05 & abs(log2FoldChange) > params$log_FC_threshold)
        capture.output(DE_genes_contrast, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast1", append=F)
        DE_genes_contrast <- DE_genes_contrast[order(DE_genes_contrast$padj),]
        # Save table
        capture.output(DE_genes_contrast, file="/mnt/volume/git/rnadeseq/bug/DE_genes_contrast2", append=F)
        write.table(DE_genes_contrast, file=paste("differential_gene_expression/DE_genes_tables/DE_contrast_",contname,".tsv",sep=""), sep="\t", quote=F, col.names = T, row.names = F)
        names(results_DEseq_contrast) = paste(names(results_DEseq_contrast),contname,sep="_")
        # Append to DE genes table for all contrasts
        DE_genes_df = cbind(DE_genes_df,results_DEseq_contrast)
        capture.output(DE_genes_df, file="/mnt/volume/git/rnadeseq/bug/DE_genes_df", append=F)
    }
}
...
logFC_bin = data.matrix(ifelse(abs(logFC) > params$log_FC_threshold, 1, 0))
...
df_contr <- read.table("contrast_names.txt", header=FALSE, sep="\t")
...
#TODO: This is the PA start
organism <- tolower(params$species)
...
```{r, eval=params$pathway_analysis, out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE}
    ...
    write("diff ex:", file="/home/centos/git/rnadeseq/status", append=F)
    capture.output(list.dirs(path=paste0("differential_gene_expression/")), file="/home/centos/git/rnadeseq/status", append=T)
    write("DE_genes_tables:", file="/home/centos/git/rnadeseq/status", append=T)
    capture.output(list.dirs(path=paste0("differential_gene_expression/DE_genes_tables/")), file="/home/centos/git/rnadeseq/status", append=T)
    q_list <- list()
    q_names <- c()
    ...
    for (file in contrast_files){
        #Reading DE genes list
        fname <- tools::file_path_sans_ext(basename(file))
        write("fname:", file="/home/centos/git/rnadeseq/status", append=T)
        capture.output(fname, file="/home/centos/git/rnadeseq/status", append=T)
        DE_genes <- read.csv(file = paste0("differential_gene_expression/DE_genes_tables/", file), sep="\t", header = T)
        if(nrow(DE_genes) <= 1){
            next
        }
        write("DE_genes:", file="/home/centos/git/rnadeseq/status", append=T)
        capture.output(DE_genes, file="/home/centos/git/rnadeseq/status", append=T)
        q = as.character(DE_genes$Ensembl_ID)
        write("q:", file="/home/centos/git/rnadeseq/status", append=T)
        capture.output(q, file="/home/centos/git/rnadeseq/status", append=T)
        q_list <- append(q_list, list(q))
        q_names <- append(q_names, fname)
    }
    datasources <- c("KEGG","REAC")
    names(q_list) <- q_names
    write("q_names:", file="/home/centos/git/rnadeseq/status", append=T)
        capture.output(file, file="/home/centos/git/rnadeseq/status", append=T)
    capture.output(q_names, file="/home/centos/git/rnadeseq/qlist")
    if (!length(q_list)) {
        stop("list empty?!?")
    }
    #gost query
