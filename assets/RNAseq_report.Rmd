...
if (!(isNot(params$path_contrast_list))) {
    ...
    metadata_cond <- metadata_save
    for (i in 1:nrow(contrasts)) {
        if (grepl("condition_", contrasts[i,1], fixed = TRUE)) {
            cond_name <- contrasts[i,1]
            for (j in 2:ncol(contrasts)) {
                if (!any(metadata_cond[cond_name]==contrasts[i,j])) {
                    stop(paste("Could not find value", contrasts[i,j], "for contrast", metadata_cond[cond_name], ", please check metadata sheet and contrast list!"))
                }
            }
        }
    }
...
```{r volcano_plot, out.width="100%", out.height="100%"}
    ...
    # the following two lines set the position of the axis descriptions (ylab and xlab)
    pg[['x']][['layout']][['annotations']][[2]][['x']] <- 5
    pg[['x']][['layout']][['annotations']][[1]][['y']] <- -20
...
for (file in contrast_files){
    ...
    if (nrow(pathway_gostres) > 0){ #if there are enriched pathways
        ...
        df_subset <- data.frame(Pathway_name = df$short_name, Pathway_code = df$term_id, DE_genes = df$intersection_size, DE_genes_names = df$intersection, Pathway_size = df$term_size, Fraction_DE = (df$intersection_size / df$term_size), Padj = df$p_value)
