...
write(paste((Sys.time() - now)/60, "min used for read_metadata"), file="/home-link/iivow01/git/rnadeseq/error/times", append=F)
```
...
write(paste((Sys.time() - now)/60, "min used for run DESeq2"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
now <- Sys.time()
...
if (run_rlog){
    ...
    # rlog transformation
    write.table(assays(cds)[["counts"]], file="/home-link/iivow01/git/rnadeseq/error/counts_before_rlog.tsv", sep="\t", quote=F)
    ...
    write.table(rld_names, "/home-link/iivow01/git/rnadeseq/error/rld.tsv", append = FALSE, quote = FALSE, sep = "\t",eol = "\n", na = "NA", dec = ".", row.names = F,  qmethod = c("escape", "double"))

    # save table to another variable if pathway analysis
    if (params$pathway_analysis) {
        norm_counts <- rld_names
        rownames(norm_counts) <- norm_counts$Ensembl_ID
        write.table(norm_counts, file="/home-link/iivow01/git/rnadeseq/error/norm_countsxy.tsv", sep="\t", quote=F)
    }

} else {

    # vst transformation
    write.table(assays(cds)[["counts"]], file="/home-link/iivow01/git/rnadeseq/error/counts_before_vst.tsv", sep="\t", quote=F)
    ...
    write.table(vsd_names, "/home-link/iivow01/git/rnadeseq/error/vsd.tsv", append = FALSE, quote = FALSE, sep = "\t",eol = "\n", na = "NA", dec = ".", row.names = F, qmethod = c("escape", "double"))
...
write(paste((Sys.time() - now)/60, "min used for transform and bla"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
now <- Sys.time()
...
write(paste((Sys.time() - now)/60, "min used for diag. plots"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
...
\
"))
write(paste((Sys.time() - now)/60, "min used for mqc"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
...
write(paste((Sys.time() - now)/60, "min used for PCA"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
...
write(paste((Sys.time() - now)/60, "min used for contrast calc"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
...
write(paste((Sys.time() - now)/60, "min used for boxplot stuff"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
nownow <- Sys.time()
```
...
for (file in contrast_files){
    ...
    write(paste((Sys.time() - nownow)/60, "min used for weird slow stuff"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
    ...
    if (nrow(pathway_gostres) > 0){ #if there are enriched pathways
        ...
        for (df in res){
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                capture.output(df, file="/home-link/iivow01/git/rnadeseq/error/df.tsv", sep="\t", quote=F)
                ...
                for (i in c(1:nrow(df))){
                    pathway <- df[i,]
                    capture.output(pathway, file="/home-link/iivow01/git/rnadeseq/error/pathway.tsv", sep="\t", quote=F)

                    gene_list <- unlist(strsplit(pathway$intersection, ","))
                    write(paste((Sys.time() - now)/60, "min used for PA before mat stuff"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)

                    capture.output(gene_list, file="/home-link/iivow01/git/rnadeseq/error/gene_list")
                    mat <- norm_counts[gene_list, ]
                    write.table(mat, file="/home-link/iivow01/git/rnadeseq/error/mat.tsv", sep="\t", quote=F)
                    write.table(norm_counts, file="/home-link/iivow01/git/rnadeseq/error/norm_countsss.tsv", sep="\t", quote=F)
...
\
"))
write(paste((Sys.time() - now)/60, "min used for all PA stuff"), file="/home-link/iivow01/git/rnadeseq/error/times", append=T)
