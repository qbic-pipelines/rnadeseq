...
DE_all$logpval[DE_all$logpval > 16] <- 17 # Set high values to max value of 17
...
pg <- ggplot(DE_all, aes(x=log2FoldChange, y=logpval, text=paste("Gene: ", gene_name,
        ...
        geom_point(alpha=0.5, width = 0.2, aes(color=ifelse(DE_all$color == "red", "Differentially expressed features", "Non-differentially expressed features"))) +
        ...
        scale_y_continuous(limits = c(1, max(DE_all$logpval))) +
...
allgenes_files <- list.files(path=paste0(wd, "/differential_gene_expression/allgenes/"))
volcano_list <-  c() #vector("list", length = length(allgenes_files))
contrast_list <- c() #vector("list", length = length(allgenes_files))
...
for (file in allgenes_files){
    ...
    DE_genes[which(DE_genes[['padj']] == 0), 'padj'] <- min(
    DE_genes[which(DE_genes[['padj']] != 0), 'padj'], na.rm = TRUE) * 10^-1

    # Imputing missing values with 1 so they do not appear as significant
    # DE_genes$padj[is.na(DE_genes$padj)] <- 1
    # Replace 0 by very small number, as -log10(0) = Inf
    # DE_genes[which(DE_genes[['pvalue']] == 0), 'pvalue'] <- 1e-6
    # DE_genes[which(DE_genes[['padj']] == 0), 'padj'] <- 1e-6
    ...
    # Scale high -log10(padj) to smaller range
    # DE_genes$logpval[DE_genes$logpval > 60] <- min_max_normalization(DE_genes$logpval > 60)
    DE_genes <- DE_genes %>% mutate(color = ifelse((abs(DE_genes$log2FoldChange) >= params$logFC_threshold) &
                        (DE_genes$logpval >= -log10(params$adj_pval_threshold)),
                        "red", "black"))
    log2FoldChange_min <- min(DE_genes$log2FoldChange)
    log2FoldChange_max <- max(DE_genes$log2FoldChange)
    log2FoldChange_quotient <- length(seq(log2FoldChange_min, log2FoldChange_max, 10))

    pg <- ggplot(DE_genes, aes(x=log2FoldChange, y=logpval, text=paste("Gene: ", gene_name,
                                                                        "<br>", "Log2FC: ", formatC(log2FoldChange, digits=2),
                                                                        "<br>", "-Log10padj: ", formatC(truelogpval, digits=2, format='f')))) +
            geom_point(alpha=0.5, aes(colour=ifelse(color == "red", "DE features",
                                                                    "Non_DE features")),
                                                                    na.rm=TRUE) +
            ...
            theme(panel.spacing = unit(ifelse(length(allgenes_files)>2, 0.1, 0.6), "lines")) +
            scale_x_continuous(breaks = seq(-200, 200, ifelse(log2FoldChange_quotient<3, 1, 5))) +
            ggtitle(paste("DE results", DE_genes$contrast[1]),
    ...
    if (isProvided(params$path_genelist)) {
        ...
        ggsave(filename = file.path(wd, "differential_gene_expression", "plots", "volcano_plots", paste0(fname, "_enhanced.png")),
            plot = p,
            width = 9,
            height = 8)
...
<!-- Put contrasts in tabs in Report -->
