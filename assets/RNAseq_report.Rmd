...
par(oma=c(3,3,3,3))
dist_heatmap <- heatmaply(sampleDistMatrix, dendrogram = "both", col=colors, grid_color = "grey", fontsize_row=label_size(nrow(sampleDistMatrix)), fontsize_col=label_size(ncol(sampleDistMatrix)))
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)))
                        dev.off()
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        ...
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific heatmap of normalized expression values for genes", scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)))
        dev.off()
...
if (isProvided(params$custom_gmt)){
    ...
    #TODO: is edgeR still relevant? It does not even exist in the env
}
